...
d %>% filter(measurement == "cov") %>% select(-measurement) -> cvr2
d %>% filter(measurement == "medhei") %>% select(-measurement) %>% 
  filter(plot_size != "c") -> medh2
d %>% filter(measurement == "maxhei") %>% select(-measurement) %>% 
  filter(plot_size != "c") -> maxh2
...
bind_rows(medh, medh2) %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "median_height") %>% 
  filter(!is.na(median_height)) %>% 
  mutate(median_height = as.numeric(gsub(",",".",median_height))) -> medh

bind_rows(maxh, maxh2) %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "max_height") %>% 
  filter(!is.na(max_height)) %>% 
  mutate(max_height = as.numeric(gsub(",",".",max_height))) -> maxh

bind_rows(flow, flow2) %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "reproduction") %>% 
