...
ss <- "https://docs.google.com/spreadsheets/d/1wC5U6EQjNLaW9uTs3Va6NNBTt9uAhgCVs95Aryo5Qhg/edit#gid=0"
...
d <- d[-1,]
d <- d %>% select(Plot:Viola_palustris)
...
cvr %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "cvr") %>% 
  pull(cvr) %>% unique()

medh %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "medh") %>% 
  pull(medh) %>% unique()
...
maxh %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "maxh") %>% 
  pull(maxh) %>% unique()

maxh %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "maxh") %>% 
  filter(maxh == "")

flow %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "flow") %>% 
  pull(flow) %>% unique()
...
write_csv(allall, "trait_data/mattavarri_vascular_all_2023.csv")
