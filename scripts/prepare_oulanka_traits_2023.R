...
ss <- "https://docs.google.com/spreadsheets/d/1E4Y2pPpQYCLDevOnbEOwg2cdqQtmbI7E9jXZg5PFvok/edit#gid=0"
...
ss <- "https://docs.google.com/spreadsheets/d/1Djkkls6GjrOk5Bt6nojSxLYICJcQnnFS4iLVX2DQncA/edit#gid=0"
...
cvr %>% 
  ...
  rename(plot = Plot,
         setting = `Study_setting`,
         plot_type = `plot_size`) %>% 
  select(plot:Asteraceae) %>% 
  pivot_longer(cols = Betula_pubescens:Asteraceae,names_to = "species",values_to = "cvr") %>% 
  filter(cvr > 0) %>% 
...
write_csv(allall, "trait_data/oulanka_vascular_all_2023.csv")
