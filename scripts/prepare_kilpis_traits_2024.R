...
la <- la %>% 
  filter(!(id == 943 & Area > 2)) # Manually remove errors in the scanning
...
d1 %>% filter(!site %in% c("MI259","RA068","RA009","EXT")) %>% add_count(site, species) %>% filter(n > 1)
d1 %>% filter(duplicated(d1 %>% select(site, species))) %>% pull(site) %>% unique()
d1 %>% filter(duplicated(d1 %>% select(site, species, date)))
...
# From Pekka
ss <- "https://docs.google.com/spreadsheets/d/1o7KTvarBCJ_P94CK1pDAAgTxIZajqVYP3_RcQLRB72A/edit?gid=0#gid=0"
...
d <- d[-1,]
d <- d %>% select(Plot:Viscaria_alpina) %>% 
...
medh %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "median_height") %>% 
  filter(!is.na(median_height)) %>% 
  mutate(median_height = as.numeric(gsub(",",".",median_height))) -> medh

maxh %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "max_height") %>% 
  filter(!is.na(max_height)) %>% 
  mutate(max_height = as.numeric(gsub(",",".",max_height))) -> maxh

flow %>% pivot_longer(cols = Betula_nana:ncol(.), names_to = "species", values_to = "reproduction") %>% 
...
right_join(all %>% rename(site = plot) %>% rename(date2 = date) %>% mutate(date2 = ymd(date2)),
           d4) %>% 
  filter(!site %in% c("RA009","RA068")) %>% 
  filter(startsWith(site,"RA") | startsWith(site,"MAL") | startsWith(site,"AIL") | startsWith(site,"SAL")) %>% 
...
write_csv(allall, "trait_data/kilpis_vascular_all_2024.csv")
